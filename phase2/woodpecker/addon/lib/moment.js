...
const updateFrequencies = function(name){
  ...
  for (let i in names){
    ...
    data.frequency = (data.count / (timer.elapsedTime() + 1)) || 0;
    data.totalFrequency = (data.count / timer.elapsedTotalTime()) || 0;
    data.effFrequency = (data.effCount / (timer.elapsedTime("intervention")+1)) || 0;
    data.effTotalFrequency = (data.effCount / timer.elapsedTotalTime("intervention")) || 0;
